document.addEventListener("DOMContentLoaded", function () {
    ...
    const n = function (e, s) {
        ...
        for (var i = e.length; i--; ) {
            ...
            "asw-filter" == s && t.states.contrast == o.key && (l = !0),
                (n += `\n<div class="asw-btn ${s || ""} ${l ? "asw-selected" : ""}" role="button" aria-pressed="false" data-key="${o.key}" arai-label="${o.label}" title="${o.label}"><span class="material-icons">${o.icon}</span>${
                    o.label
                }\n</div>`),
                a.push(o.icon);
        }
    ...
    const script = document.getElementById('accessibility-script');
    if (script) {
    	var nonce = script.getAttribute('data-nonce');
			
		// Create the link element
		const link = document.createElement('link');
		link.href = `https://fonts.googleapis.com/icon?family=Material+Icons&text=${a.toString()}`;
		link.rel = 'stylesheet';
		...
		style.textContent = `
			...
			.asw-widget { user-select: none; font-weight: 400; -webkit-font-smoothing: antialiased; }
			.asw-widget * { box-sizing: border-box; }
			.asw-menu-btn { bottom: 20px; background: #0048ff !important; box-shadow: 0 5px 15px 0 rgb(37 44 97 / 15%), 0 2px 4px 0 rgb(93 100 148 / 20%); border-radius: 50%; align-items: center; justify-content: center; transform: translateY(0); width: 64px; height: 64px; display: flex; fill: white; cursor: pointer; }
			.asw-menu-btn svg { width: 34px; height: 34px; min-height: 34px; min-width: 34px; max-width: 34px; max-height: 34px; background: transparent !important; }
			.asw-menu-btn:hover { transform: scale(1.05); }
			.asw-menu { display: none; top: 20px; border-radius: 8px; box-shadow: -1px 0 20px -14px #000; opacity: 1; overflow: hidden; background: #fff; width: 500px; line-height: 1; font-size: 14px; height: calc(100vh - 40px - 75px); letter-spacing: .015em; }
			...
			.asw-btn.asw-selected { background: #0048ff; color: #fff; border-color: #0048ff; }
			.asw-footer { position: absolute; bottom: 0; left: 0; right: 0; background: #0334b1; padding: 16px; text-align: center; color: #fff; }
			.asw-footer a { text-decoration: underline; color: #fff; background: transparent !important; }
			.asw-menu-content { overflow-y: auto; max-height: calc(100% - 80px); color: #000; }
			...
			@media only screen and (max-width: 560px) { .asw-menu { width: calc(100vw - 20px); left: 10px; } .asw-btn { width: calc(50% - 8px); } }
		...
		// Example HTML content (without inline styles)
		r.innerHTML = `
			<div class="asw-widget">
				<div class="asw-menu-btn" title="Open Accessibility Menu" role="button" aria-expanded="false">
					<svg xmlns="http://www.w3.org/2000/svg" class="size-34" viewBox="0 0 24 24" width="34px" height="34px">
						<path d="M0 0h24v24H0V0z" fill="none"/>
						<path d="M20.5 6c-2.61.7-5.67 1-8.5 1s-5.89-.3-8.5-1L3 8c1.86.5 4 .83 6 1v13h2v-6h2v6h2V9c2-.17 4.14-.5 6-1l-.5-2zM12 6c1.1 0 2-.9 2-2s-.9-2-2-2-2 .9-2 2 .9 2 2 2z"/>
					</svg>
				</div>
				<div class="asw-menu">
					<div class="asw-menu-header"> Accessibility Menu
						<div>
					...
					<div class="asw-menu-content">
						<div class="asw-card mt-7">
							<div class="asw-card-title"> Content Adjustments </div>
							<div class="asw-adjust-font">
								<div class="label">
									<span class="material-icons me-4"> format_size </span> Adjust Font Size
								</div>
								<div>
									<div class="asw-minus" data-key="font-size" role="button" aria-pressed="false">
										<span class="material-icons"> remove </span>
									</div>
									<div class="asw-amount">${t.states.fontSize && 1 != t.states.fontSize ? `${parseInt(100 * t.states.fontSize)}%` : "Default"}</div>
					...
					<div class="asw-footer">
						<a href="#">GSF Accessibility Statement</a>
					</div>
				</div>
				...
				if (t.states["readable-guide"]) {
					if (!i) {
						...
						style.textContent = `
							...
							.asw-rg-top { /* Additional styles if needed */ }
							.asw-rg-bottom { top: auto; bottom: 0; }
						`;
						...
						o.setAttribute("class", "asw-rg-container"),
							(o.innerHTML =
							'<div class="asw-rg asw-rg-top"></div><div class="asw-rg asw-rg-bottom"></div>\n');
		...
		var f = function (s) {
			...
			a.classList.contains("asw-filter")
				? (document.querySelectorAll(".asw-filter").forEach(function (t) {
				  ...
				  t.states.contrast && (a.classList.add("asw-selected"), a.setAttribute("aria-pressed", "true")),
				  p(t.states.contrast))
				: ((t.states[n] = !t.states[n]), a.classList.toggle("asw-selected", t.states[n]), a.setAttribute("aria-pressed", t.states[n] ? "true" : "false"), u()),
				e();
		};
		const h = function (e, s) {
			let a = !1;
			!s && e && ((a = e.currentTarget), (s = parseFloat(t.states.fontSize) || 1), a.classList.contains("asw-minus") ? (s -= 0.1) : (s += 0.1), (s = Math.max(s, 0.1)), (s = Math.min(s, 2)), (s = parseFloat(s.toFixed(2)))),
				document.querySelectorAll("h1,h2,h3,h4,h5,h6,p,a,dl,dt,li,ol,th,td,span").forEach(function (t) {
